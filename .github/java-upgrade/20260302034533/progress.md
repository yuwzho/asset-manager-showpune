# Upgrade Progress
  ...
  - ###
    ...
    - Upgrade Java version from 17 to 21
      - Updated `java.version` property in root pom.xml from `17` to `21`
      - Replaced `Paths.get()` with `Path.of()` in LocalFileStorageService.java (modern Java API)
      - Replaced `Paths.get()` with `Path.of()` in LocalFileProcessingService.java (modern Java API)
      - Removed unused `java.nio.file.Paths` import from LocalFileProcessingService.java
      - Expanded wildcard import `java.nio.file.*` to explicit imports in LocalFileStorageService.java
    </details>
