# Upgrade Progress
  ...
  - ###
    ...
    - Modernize deprecated NIO APIs
      - In `LocalFileStorageService.java`: replaced wildcard `java.nio.file.*` import with explicit imports, replaced `Paths.get()` with `Path.of()`
      - In `LocalFileProcessingService.java`: removed unused `java.nio.file.Paths` import, replaced `Paths.get()` with `Path.of()`
    </details>
